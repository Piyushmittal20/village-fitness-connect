import React, { useState } from "react";
import { useListDietPlans } from "@workspace/api-client-react";
import { LoadingSpinner, ErrorState, EmptyState } from "@/components/States";
import { DietPlanCard } from "@/components/Cards";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Input } from "@/components/ui/input";
import { Search } from "lucide-react";

export default function DietPlansList() {
  const [goal, setGoal] = useState<string>("all");
  const [search, setSearch] = useState("");
  
  const { data: plans, isLoading, error, refetch } = useListDietPlans();

  const filteredPlans = plans?.filter(p => {
    const matchesGoal = goal === "all" || p.goal.toLowerCase() === goal.toLowerCase();
    const matchesSearch = p.title.toLowerCase().includes(search.toLowerCase());
    return matchesGoal && matchesSearch;
  }) || [];

  return (
    <div className="container mx-auto px-4 py-12">
      <div className="mb-12 text-center max-w-2xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-display font-bold mb-4">Diet Plans</h1>
        <p className="text-lg text-muted-foreground">
          Nutrition guides crafted by experts to fuel your rural fitness journey.
        </p>
      </div>

      <div className="mb-8 max-w-2xl mx-auto flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <Input 
            placeholder="Search plans..." 
            className="pl-10 h-12 rounded-full border-border bg-card text-base"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        <div className="w-full sm:w-48 shrink-0">
          <Select value={goal} onValueChange={setGoal}>
            <SelectTrigger className="h-12 rounded-full bg-card">
              <SelectValue placeholder="Goal" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Goals</SelectItem>
              <SelectItem value="muscle_gain">Build Muscle</SelectItem>
              <SelectItem value="weight_loss">Lose Weight</SelectItem>
              <SelectItem value="maintenance">Maintenance</SelectItem>
              <SelectItem value="endurance">Endurance</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>

      {isLoading && <LoadingSpinner />}
      {error && <ErrorState retry={() => refetch()} />}
      
      {!isLoading && !error && filteredPlans.length === 0 && (
        <EmptyState 
          title="No diet plans found" 
          description="We couldn't find any plans matching your criteria." 
        />
      )}

      {!isLoading && !error && filteredPlans.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPlans.map((plan) => (
            <DietPlanCard key={plan.id} plan={plan} />
          ))}
        </div>
      )}
    </div>
  );
}
